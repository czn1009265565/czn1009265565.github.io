...
csv_path = "loader.csv"
csv_loader = CSVLoader(csv_path, source_column="Location", encoding="utf-8")
...
加载Excel文件则需要安装依赖库 `pip install "unstructured[xlsx]"`

```python
from langchain.document_loaders import UnstructuredExcelLoader

loader = UnstructuredExcelLoader(file_path="loader.xlsx")
docs = loader.load()
print(docs)
```
...
## Docx2txtLoader
用于加载 Microsoft Word 文档（.docx 格式） 的文档加载器。
基于 docx2txt 库实现，能够提取文档中的文本内容（包括段落、列表、表格中的文字等），但不支持图片或复杂格式的保留
...
# 文档会被转换为 LangChain 的 Document 对象
