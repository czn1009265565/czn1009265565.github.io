...
### 直接绑定
将事件直接绑定在特定元素上，绑定时的回调函数会固定引用该元素初始状态的数据。若元素数据后续发生变化，事件处理函数仍可能使用绑定时缓存的数据
...
### 事件委托
将事件绑定在父级或祖先元素上，通过事件冒泡（Event Bubbling）机制捕获子元素触发的事件。
由于事件处理函数在事件触发时才通过事件对象（如 event.target）动态判断目标元素，因此能获取到触发瞬间的最新数据
